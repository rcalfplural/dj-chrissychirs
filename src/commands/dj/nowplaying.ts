...
async function execute({message, args, client}: ICommandArgs){
    try{
        ...
        const embed = EmbedConstructor({ title: String(song.video_details.title), hasAuthor: true, customAuthor: "DJ Chrissy Chris está tocando agora em "+await GetServerName(message), hasFooter: false });
